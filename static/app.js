...
const specGroups = [
  ...
  },
  {
    ...
    keys: [
      ...
      "ProcessorManufactureProcess",
      "ProcessorTechnologyNode",
      "ProcessorNeuralEngine",
      "ProcessorNeuralEngineCores",
      "Coprocessor",
      "CoprocessorSpeed",
      "Cache",
      "SystemBus",
...
let favsCache;
try {
  favsCache = JSON.parse(localStorage.getItem("favourites")) || {};
} catch {
  favsCache = {};
}
...
async function resolveSlugAndShow(slug) {
  ...
  for (let len = words.length; len >= 1; len--) {
    ...
    for (const item of results) {
      if (
        slugify(item.modelName) === slug ||
        itemSlug(item.modelName, item.uuid) === slug
      ) {
...
async function loadSideboard() {
  let entries;
  try {
    entries = await fetchJSON("/api/sideboard");
  } catch {
    return;
  }
...
async function selectSideboardEntry(index, fromRouter) {
  ...
  showView("items-view");
  window.scrollTo(0, 0);
  document.title = `${entry.name} — Pommeraie`;
...
function showFavourites(fromRouter) {
  ...
  showView("items-view");
  window.scrollTo(0, 0);
  document.title = "Favourites — Pommeraie";
  ...
  if (items.length === 0) {
    grid.innerHTML =
      '<div class="no-results">No favourites yet. Browse products and tap ★ to save them here.</div>';
    return;
  }
...
async function showDetail(uuid, fromRouter, slug) {
  ...
  let item;
  try {
    item = await fetchJSON(`/api/items/${uuid}`);
  } catch {
    return;
  }
  if (item.ModelName && item.UUID) {
    itemSlug(item.ModelName, item.UUID);
    document.title = `${item.ModelName} — Pommeraie`;
  }
  ...
  for (const group of specGroups) {
    ...
    for (const key of group.keys) {
      if (
        ...
        rows.push(
          `<div class="spec-row"><div class="spec-label">${formatLabel(key)}</div><div class="spec-value">${formatValue(item[key])}</div></div>`,
        );
        usedKeys.add(key);
      }
    }
...
function goBack() {
  if (window.history.length > 1) {
    ...
    navigate(
      `#/browse/${slugify(sideboardEntries[currentSideboardIndex].name)}`,
    );
  } else {
    navigate("#/");
  }
...
async function performSearch(query, fromRouter) {
  ...
  document.title = `Search: ${query} — Pommeraie`;
